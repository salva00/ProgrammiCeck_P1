...
template<typename T>
Matrix<T>::Matrix(const vector<vector<T>>& input) : rows(input.size()), cols(input[0].size()) {
...
Matrix<T> Matrix<T>::operator*(Matrix<T> mult) {
...
Matrix<T> Matrix<T>::operator*(T mult) {
  Matrix<T> res = *this;
  for(size_t i = 0; i < this->rows; i++) {
    for(size_t j = 0; j < this->cols; j++) {
